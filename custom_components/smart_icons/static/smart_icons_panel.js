...
`;var It=0;function T(){return It++}var y=class extends f{constructor(){super(...arguments);this.errorMessage="";this.working=this.blankState();this.observedStates=[];this._observedStatesCache=new Map;this._observedStatesEntityId="";this.cancelClicked=()=>{this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))};this.addGlob=()=>{this.working={...this.working,targetGlobs:[...this.working.targetGlobs,{value:"",_key:T()}]}};this.addThreshold=()=>{this.working={...this.working,thresholds:[...this.working.thresholds,{lt:0,color:"",icon:"",_key:T()}]}};this.addMapping=()=>{this.working={...this.working,mapping:[...this.working.mapping,{key:"",color:"",icon:"",_key:T()}]}};this.save=()=>{let t=this.serialize();this.dispatchEvent(new CustomEvent("save",{detail:t,bubbles:!0,composed:!0}))}}connectedCallback(){super.connectedCallback();for(let t of["ha-icon-picker","ha-selector"])customElements.get(t)||customElements.whenDefined(t).then(()=>this.requestUpdate()).catch(()=>{})}willUpdate(t){t.has("rule")&&(this.working=this.rule?this.hydrate(this.rule):this.blankState());let r=this.observedSourceForAutocomplete();r!==this._observedStatesEntityId&&(this._observedStatesEntityId=r,this.refreshObservedStates(r))}observedSourceForAutocomplete(){let t=this.working.source.trim();return t||this.working.targetEntities[0]||""}async refreshObservedStates(t){if(!t||!this.hass){this.observedStates=[];return}let r=this._observedStatesCache.get(t);if(r){this.observedStates=r;return}let i=this.hass.states[t]?.state;i&&(this.observedStates=[i]);let s=new Set;i&&s.add(i);try{let a=new Date,l=new Date(a.getTime()-10080*60*1e3),h=(await this.hass.connection.sendMessagePromise({type:"history/history_during_period",start_time:l.toISOString(),end_time:a.toISOString(),entity_ids:[t],minimal_response:!0,no_attributes:!0}))?.[t];if(Array.isArray(h))for(let d of h){let g=d.s??d.state;typeof g=="string"&&g&&s.add(g)}}catch{}if(t!==this._observedStatesEntityId)return;let o=[...s].sort();this._observedStatesCache.set(t,o),this.observedStates=o}renderIconField(t,r){return customElements.get("ha-icon-picker")&&this.hass?c`
        <ha-icon-picker
          .hass=${this.hass}
          .value=${t}
          .label=${"Icon"}
    ...
    `}render(){return c`
      ...
      <datalist id="smart-icons-observed-states">
        ${this.observedStates.map(t=>c`<option value=${t}></option>`)}
      </datalist>
    ...
    `}moveThreshold(t,r){let i=t+r;if(i<0||i>=this.working.thresholds.length)return;let s=[...this.working.thresholds];[s[t],s[i]]=[s[i],s[t]],this.working={...this.working,thresholds:s}}renderMapping(){let t=this.working.mapping,r=t.every(i=>!i.key.trim());return c`
      <fieldset>
        ...
        ${this.observedStates.length>0?c`<p class="fieldset-hint">
